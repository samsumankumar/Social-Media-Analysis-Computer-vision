...
# Model Deployment:
In order to run the model on local machine, use the compatible version of tensorflow (version 2.2 or higher) and python3 and run python3 application.py in the command prompt.\
If you are planning to host the flask web application on Virtual Cloud Instance, replace the localhost with 0.0.0.0 and enable port 80 on the instance to serve the application.
# Future Work:
1.Accuracy can be improved by increasing the amount of data per each class
